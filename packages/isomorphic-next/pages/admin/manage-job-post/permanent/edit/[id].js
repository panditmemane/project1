...
const JobPostPermanentUpdate = () => {
  ...
  useEffect(() => {
    const handlerJobPostDetails = async () => {
      ...
      const { status, manpower_positions, documents_uploaded, pre_ad_description, post_ad_description, ...rest } =
        response;
      const advertisement = documents_uploaded
        .filter((document) => document.doc_name === 'advertisement')
        .map((doc) => ({
          uid: doc.doc_id,
          name: doc.doc_name,
          status: 'done',
      ...
      const additional = documents_uploaded
        .filter((document) => document.doc_name === 'additional')
        .map((doc) => ({
          uid: doc.doc_id,
          name: doc.doc_name,
          status: 'done',
      ...
      const preDescriptionState = EditorState.createWithContent(preDescriptionContentState);
      const postDescriptionContentState = convertFromRaw(JSON.parse(post_ad_description));
      const postDescriptionState = EditorState.createWithContent(postDescriptionContentState);
  ...
  const onSuccessDocumentsUploaded = (data, type) => {
  ...
  const handleOnChangePosition = (value, key, name) => {
    ...
    manpower_positions[key].position = selectedPositionData[0].perm_position_master.position;
                      ...
                      <Col span={12}>
                        <Form.Item name='additional_documents' label='Additional Documents' labelCol={{ span: 24 }}>
                          <Dragger
                            ...
                            onSuccess={(data) => onSuccessDocumentsUploaded(data, 'additional')}
                          >
                            <p className='ant-upload-drag-icon'>
                              <InboxOutlined />
                            </p>
